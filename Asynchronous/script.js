...
2. Do 'reverse geocoding' of the provided coordinates. Reverse geocoding means to
 convert coordinates to a meaningful location, like a city and country name.
...
you will get this error with code 403. This is an error with the request.
 Remember, fetch() does NOT reject the promise in this case. So create an error to reject
...
const renderCountry = function (data) {
  ...
  <article>
    <div class="country__data">
      <h2 class="city__name">${data.city}</h2>
...
const whereAmI = function (lat, lng) {
  fetch(
    ...
    .then(data => {
      console.log(data);
      console.log(
        `You are in ${data.city}, ${data.countryName},${data.continent}`
      );

      return fetch(
        `https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${lng}&continent=${data.continent}`
      );
    })
    .then(res => {
